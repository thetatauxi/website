...
export function HomeOurNetwork() {
  ...
  // Start company logos halfway through for initial rendering
  const totalLogos = companyLogos.length
  const logosPerRow = 8
  const numberOfRows = Math.ceil(totalLogos / logosPerRow)

  // Compute halfway index
  const halfwayIndex = Math.floor(totalLogos / 2)

  // Build new company logos array that starts halfway through, wraps around
  const companyLogosRotated = [
    ...companyLogos.slice(halfwayIndex),
    ...companyLogos.slice(0, halfwayIndex)
  ]

  // Fill remaining slots with duplicates from the start to ensure equal rows
  const paddedLogos = [...companyLogosRotated]
  while (paddedLogos.length < numberOfRows * logosPerRow) {
    paddedLogos.push(companyLogosRotated[paddedLogos.length % companyLogosRotated.length])
  }
    ...
    <section className="relative pt-32 pb-20 overflow-hidden bg-white">
      ...
      <div className="w-full">
        {rows.map((row, rowIndex) => (
          ...
          >
            <div
              className={`inline-flex logo-track-centered ${isPaused ? "paused" : ""}`}
              style={{
                width: "fit-content",
                whiteSpace: "nowrap",
              }}
